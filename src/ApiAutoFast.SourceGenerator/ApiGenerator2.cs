...
[Generator]
public class ApiGenerator2 : IIncrementalGenerator
{
    static ApiGenerator2()
    {
   ...
   // private static readonly EndpointTargetType[] _endpointTargetNames = (Enum.GetValues(typeof(EndpointTargetType)) as EndpointTargetType[])!;
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput(ctx => ctx.AddSource(
            ...
            SourceText.From(SourceEmitter2.AutoFastEndpointsAttribute, Encoding.UTF8)));

        context.RegisterPostInitializationOutput(ctx => ctx.AddSource(
            "AutoFastContextAttribute.g.cs",
            SourceText.From(SourceEmitter2.AutoFastContextAttribute, Encoding.UTF8)));
    ...
    private static void Execute(Compilation compilation, ImmutableArray<SemanticTargetInformation> semanticTargets, SourceProductionContext context)
    {
        ...
        {
            var entityResult = SourceEmitter2.EmitEntityModels(entityGenerationConfig.Namespace, entityConfig);
            context.AddSource($"{entityConfig.BaseName}.g.cs", SourceText.From(entityResult, Encoding.UTF8));
        }

        var mappingRegisterResult = SourceEmitter2.EmitMappingRegister(entityGenerationConfig);
        ...
        var dbContextResult = SourceEmitter2.EmitDbContext(contextConfig, entityGenerationConfig);
        ...
        foreach (var entityConfig in entityGenerationConfig.EntityConfigs)
        {
            ...
            {
                var requestModelsResult = SourceEmitter2.EmitModelTarget(entityGenerationConfig.Namespace, entityConfig, requestEndpointPair.RequestModel);
            ...
            var mappingProfileResult = SourceEmitter2.EmitMappingProfile(entityGenerationConfig.Namespace, entityConfig);
            ...
            foreach (var requestEndpointPair in _requestEndpointPairs)
            {
                ...
                var requestModelsResult = SourceEmitter2.EmitEndpoint(entityGenerationConfig.Namespace, endpointConfig, contextConfig.Name);
                context.AddSource($"{endpointConfig.Name}.g.cs", SourceText.From(requestModelsResult, Encoding.UTF8));
            }
        }


        //var mappingRegisterResult = SourceEmitter2.EmitMappingRegister(contextConfig);
        //context.AddSource("MappingRegister.g.cs", SourceText.From(mappingRegisterResult, Encoding.UTF8));

        //if (generationConfig.EndpointsGeneration.HasValue && generationConfig.EndpointsGeneration.Value.EntityEndpoints.Length > 0)
        //{
        //    var endpointsConfig = generationConfig.EndpointsGeneration!.Value;

        //    var dbContextResult = SourceEmitter2.EmitDbContext(contextConfig, endpointsConfig);
        //    context.AddSource($"{contextConfig.Name}.g.cs", SourceText.From(dbContextResult, Encoding.UTF8));

        //    var mappingRegisterExtendedResult = SourceEmitter2.EmitMappingRegisterExtended(contextConfig, endpointsConfig);
        //    context.AddSource("MappingRegisterExtended.g.cs", SourceText.From(mappingRegisterExtendedResult, Encoding.UTF8));

        //    foreach (var entityEndpoint in endpointsConfig.EntityEndpoints)
        //    {
        //        var entityResult = SourceEmitter2.EmitEntityModels(contextConfig.Namespace, entityEndpoint);
        //        context.AddSource($"{entityEndpoint.Entity}.g.cs", SourceText.From(entityResult, Encoding.UTF8));

        //        if (Type.GetType($"{entityEndpoint.BaseName}Response") is not null)
        //        {

        //        }

        //        var requestModelsResult = SourceEmitter2.EmitRequestModels(contextConfig.Namespace, entityEndpoint);
        //        context.AddSource($"{entityEndpoint.BaseName}Request.g.cs", SourceText.From(requestModelsResult, Encoding.UTF8));

        //        //        var mappingProfileResult = SourceEmitter.GenerateMappingProfiles(generationConfig);
        //        //        context.AddSource("AutoFastMappingProfiles.g.cs", SourceText.From(mappingProfileResult, Encoding.UTF8));

        //        //        var endpointsResult = SourceEmitter.GenerateEndpoints(generationConfig);
